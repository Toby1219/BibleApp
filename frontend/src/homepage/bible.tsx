...
export default function ReaderPage() {
  ...
  const handleBookMark = () => {
    apiRequest("post", "/auth/save_bookmark", {
      book_name: book,
      chapter: chapter,
    }).then((resp) => {
      if (resp.status === 200) {
  ...
  useEffect(() => {
    ...
    const verses = bookmarkBooks
      .filter((b) => b.book === book && b.chapter === chapter)
      .map((b) => b.verse);
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 md:px-10 pt-10 sm:pt-14 pb-16">
        ...
        >
          ...
          {CHAPTER.verses.map((v, i) => (
            <div key={v.num} className="flex items-start gap-2 group">
              <button
              ...
              <span id={`verse-${v.num}`}>
                {i !== 0 && (
                  <sup className="bh-verse-num text-[#7B2942]">{v.num}</sup>
                )}
